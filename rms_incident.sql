...
SELECT
	...
	,IE.callNumber AS 'call_number'
	,description_Description AS 'description'
	,status_Description AS 'status'
	,dateReported AS 'report_date'
	,CONVERT(VARCHAR(10), startDate, 101) AS 'date'
	,CONVERT(VARCHAR(10), startDate, 108) AS 'time'
	,endDate AS 'end_date'
	,disposition_Description AS 'disposition'
	,CASE
	...
	END AS 'location'
	,CASE
		WHEN address_streetAddress IN ('800 W PITKIN ST', 
									   '820 W PITKIN ST', 
									   '816 W PITKIN ST', 
									   '810 W PITKIN ST', 
									   '511 W LAKE ST', 
									   ...
									   '1600 W PLUM ST', 
									   '1700 W PLUM ST', 
									   '1009 W LAUREL ST') THEN 'Residential'
		ELSE 'Other'
...
WHERE
	dateReported > @ThreeYearsAgo
	AND
	description_Description NOT IN('ERROR CASE')
)
SELECT
	*
FROM
